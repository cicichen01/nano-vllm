...
## Next (Step 7) — candidates
- **A. End-to-end capstone** — trace one `generate()` through every component (integration sequence diagram).
- **B. Optimization roadmap deep-dive** — expand `NANO_VLLM_NOTES.md` backlog into design sketches (spec decoding,
  async scheduling, chunked-prefill/decode overlap, quantization, prefix-cache eviction, disaggregation, MoE/EP).
